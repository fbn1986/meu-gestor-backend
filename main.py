...
def add_expense(db: Session, user: User, expense_data: dict):
    ...
    db.add(new_expense)
    db.commit()
    logging.info("Despesa salva com sucesso!")

def get_expenses_summary(db: Session, user: User, period: str) -> float | None:
...
def call_dify_api(user_id: str, text_query: str) -> dict | None:
    """Envia uma consulta para o agente Dify e retorna a resposta em JSON."""
    ...
    try:
        if action == "register_expense":
            ...
            if deleted_expense:
                ...
                formatted_valor = f"{valor_f:.2f}".replace('.', ',')
                confirmation = f"🗑️ Despesa anterior ('{descricao}' de R$ {formatted_valor}) foi removida."
                send_whatsapp_message(sender_number, confirmation)
            else:
...
# Permite rodar o servidor com `python main.py`
